...
meta <- read.csv("new_meta_Velmeshev_2022.csv", header=T, sep=",", as.is=T, row.names=1)
rownames(meta) <- meta$cell

meta_2nd_trim <- subset(meta, age=="2nd trimester")

mat_2nd_trim_1 <- fread("Velmeshev_outs/2nd_trimester_1.tsv.gz")
mat_2nd_trim_2 <- fread("Velmeshev_outs/2nd_trimester_2.tsv.gz")
...
DimPlot(velm_2nd_trim, reduction = "umap", group.by = "cluster_final")
...
mat_10_20_years <- fread("10_20_years.tsv.gz")
...
saveRDS(velm_10_20_years, paste0("Seurat_UCSC/", velm_10_20_years@project.name, ".rds"))
...
velm_10_20_years <- readRDS("Velmeshev_2022_10_20_years.rds")
...
DimPlot(velm_10_20_years, reduction = "umap", group.by = "cluster_final")
