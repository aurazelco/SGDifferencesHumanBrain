...
Idents(velm_1st_year) <- "sex_ct"

min_cells <- 100

main_deg <- paste0(main, "Velmeshev_2022_0_1_years/outputs/")
...
for (ct_type in unique(final_groups$ct)) {
  ...
  if ((id1 %in% unique(final_groups$idents)) & (id2 %in% unique(final_groups$idents))) {
    ...
    deg1 <- FindMarkers(velm_1st_year, 
                              ident.1 = id1, 
                              ident.2 = id2,
    ...
    deg2 <- FindMarkers(velm_1st_year, 
                              ident.1 = id2, 
                              ident.2 = id1,
                              logfc.threshold = 0.25,
                              min.pct = 0.1,
...
####### GENERAL VARIABLES
main_local <- "/Users/aurazelco/Desktop/Lund_MSc/Thesis/data/UCSC/DEGs_adjust_pval/Velmeshev_2022_0_1_years/outputs/"
...
PlotGeneralHeatmap(main_local, chr_1st_year, ct_order, "Velmeshev_0_1_years")
PlotSexHmp(main_local, chr_1st_year, ct_order)
...
all_genes <- read.csv("/Users/aurazelco/Desktop/Lund_MSc/Thesis/data/UCSC/DEGs_adjust_pval/Velmeshev_2022_0_1_years/tot_genes_ct_Velmeshev_2022_0_1_years.csv")
all_genes$X <- NULL
