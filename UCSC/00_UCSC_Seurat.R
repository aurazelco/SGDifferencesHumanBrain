...
####  Separate cell barcodes by age group
...
for (age in unique(meta$age)) {
  ...
  indexes <- as.numeric(rownames(meta[which(meta$age==age),]))
  indexes <- indexes + 1
  split_index <- append(split_index, list(c(1,indexes)))
}
names(split_index) <- str_replace_all(unique(meta$age), c(" "="_", "-"="_"))
...
mat <- fread("/Users/aurazelco/Desktop/Lund_MSc/Thesis/data/UCSC/UCSC_downloads/exprMtx_filt_Velmeshev_2022.tsv.gz")
colnames(mat) <- c("gene", rownames(meta))
genes <- mat[,1][[1]]
genes <- gsub(".+[|]", "", genes)
mat <- data.frame(mat[,-1], row.names=genes)

velm <- CreateSeuratObject(counts = mat, project = "Velmeshev_2022", meta.data=meta, assay = "RNA")

# Seurat tutorial https://satijalab.org/seurat/articles/pbmc3k_tutorial.html
velm[["percent.mt"]] <- PercentageFeatureSet(velm, pattern = "^MT-")
# Visualize QC metrics as a violin plot
VlnPlot(velm, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
# FeatureScatter is typically used to visualize feature-feature relationships, but can be used
# for anything calculated by the object, i.e. columns in object metadata, PC scores etc.
plot1 <- FeatureScatter(velm, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(velm, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
velm <- subset(velm, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5)
velm <- NormalizeData(velm)
velm <- FindVariableFeatures(velm, selection.method = "vst", nfeatures = 2000)
# Identify the 10 most highly variable genes
top10 <- head(VariableFeatures(velm), 10)
# plot variable features with and without labels
plot1 <- VariableFeaturePlot(velm)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1 + plot2
all.genes <- rownames(velm)
velm <- ScaleData(velm, features = all.genes)
velm <- RunPCA(velm, features = VariableFeatures(object = velm))
# Examine and visualize PCA results a few different ways
print(velm[["pca"]], dims = 1:5, nfeatures = 5)
VizDimLoadings(velm, dims = 1:2, reduction = "pca")
DimPlot(velm, reduction = "pca") 
DimHeatmap(velm, dims = 1, cells = 500, balanced = TRUE)
# NOTE: This process can take a long time for big datasets, comment out for expediency. More
# approximate techniques such as those implemented in ElbowPlot() can be used to reduce
# computation time
velm <- JackStraw(velm, num.replicate = 100)
velm <- ScoreJackStraw(velm, dims = 1:20)
JackStrawPlot(velm, dims = 1:15)
ElbowPlot(velm)
velm <- FindNeighbors(velm, dims = 1:12)
velm <- FindClusters(velm, resolution = 0.5)
# Look at cluster IDs of the first 5 cells
#head(Idents(velm), 5)
# If you haven't installed UMAP, you can do so via reticulate::py_install(packages =
# 'umap-learn')
velm <- RunUMAP(velm, dims = 1:12)
DimPlot(velm, reduction = "umap")
VlnPlot(velm, features = "XIST", group.by = "Name") + NoLegend()
ggsave(paste0(main, velm@project.name, "_XIST.pdf"))

velm@meta.data$sex <- rep("no_data", nrow(velm@meta.data))
