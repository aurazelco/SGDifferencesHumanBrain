...
for (age in unique(meta$age)) {
  split_barcodes <- append(split_barcodes, list((paste(c(age, meta[which(meta$age==age), "cell"]), collapse = ","))))
}
Reduce(intersect, split_barcodes)

sink("/Users/aurazelco/Desktop/Lund_MSc/Thesis/data/UCSC/UCSC_downloads/Velmeshev_split/Velmeshev_barcodes.txt")
...
for (age in unique(meta$age)) {
  ...
  split_index <- append(split_index, list(as.numeric(rownames(meta[which(meta$age==age),]))))
}
...
lapply(1:length(names(split_index)), function(x) write.table(
                                            t(split_index[[x]]),
                                            paste0("/Users/aurazelco/Desktop/Lund_MSc/Thesis/data/UCSC/UCSC_downloads/Velmeshev_splitVelmeshev_2022_", names(split_index)[x], ".txt"),
                                            sep=",",
                                            row.names = F,
                                            col.names = F))
