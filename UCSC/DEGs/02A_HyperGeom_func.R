...
CalcTotGenes <- function(main_dir, sex, ext, row_col) {
  df_sex <- ReadRawData(main_dir, sex, ext, row_col)
  uniq_sex <- CountGenes(df_sex)
  return(uniq_sex)
}

CalcTotGenes2 <- function(main_dir, sex, ext, row_col) {
  df_sex <- ReadRawData2(main_dir,  sex, ext, row_col)
  uniq_sex <- CountGenes(df_sex)
  return(uniq_sex)
}

# 5. Retrieve num of X and Y-DEGs
XYdeg <- function(main_dir, sex, ext, row_col) {
  XY <- ImportSignDE(paste(main_dir, "01C_num_chr", sep="/"), ext, row_col)
...
CalcHyperGeom2 <- function(main_dir, tot_genes, df_sex, chr_genes, chr, ext, row_col) {
  pval_res <- vector()
  for (ct in rownames(df_sex)) {
    pval <- phyper(
      df_sex[ct, chr] - 1,
...
HyperGeomDEGs <- function(main_dir,  sex, tot_genes, X_chr, Y_chr, ext, row_col) {
...
HyperGeomDEGs2 <- function(main_dir, sex, tot_genes, X_chr, Y_chr, ext, row_col) {
  df_sex <- XYdeg(main_dir, sex, ext, row_col)
...
PlotHyperGeom <- function(main_dir,  df_sex, sex) {
...
PlotHyperGeom2 <- function(main_dir,df_sex, sex) {
  df_melt <- melt.data.frame(df_sex[, c(4:5)])   
  df_melt$ct <- as.factor(rep(rownames(df_sex), 2))
  colnames(df_melt) <- c("chr", "pval", "ct")
  df_melt$chr <- as.factor(df_melt$chr)
...
# MAIN
SexChr <- function(main_dir, tot_genes, X_chr, Y_chr, ext, row_col) {
  df_F <- HyperGeomDEGs(main_dir,"F", tot_genes, X_chr, Y_chr, ext, row_col)
  df_M <- HyperGeomDEGs(main_dir,  "M", tot_genes, X_chr, Y_chr, ext, row_col)
  print("Only significant p-values will be plotted and saved as heatmap")
  PlotHyperGeom(main_dir, df_F, "F")
  PlotHyperGeom(main_dir, df_M, "M")
}

SexChr2 <- function(main_dir, tot_genes, X_chr, Y_chr, ext, row_col) {
  df_F <- HyperGeomDEGs2(main_dir, "F", tot_genes, X_chr, Y_chr, ext, row_col)
  df_M <- HyperGeomDEGs2(main_dir,  "M", tot_genes, X_chr, Y_chr, ext, row_col)
  print("Only significant p-values will be plotted and saved as heatmap")
  PlotHyperGeom2(main_dir, df_F, "F")
  PlotHyperGeom2(main_dir, df_M, "M")
}
