...
SaveXpar <- function(main_dir, Xpar_l) {
  dir.create(paste0(main_dir, "/01D_Xpars/"), showWarnings = FALSE)
  xpar_genes <- vector()
  df_ct <- vector()
  which_xpar <- vector()
  df_sex <- vector()
  ...
  write.csv(df_xpar, paste0(main_dir, "/01D_Xpars/Xpars_genes.csv"))
  return(df_xpar)
}
  
# 5. Plot
PlotXpar <- function(main_dir, df_xpar, ct_ordered) {
  ...
  dev.off()
  write.csv(plt_xpar, paste0(main_dir, "/01D_Xpars/Xpars_genes_table.csv"))
}

# 6. Analyze ct
XparCt <- function(main_dir, xpar1, xpar2, ct_ordered, ext, row_col) {
  ...
  if (all(lengths(XparAll)!=0L)) {
    ...
    XparAll_df <- SaveXpar(main_dir, XparAll)
    Xparplot <- PlotXpar(main_dir, XparAll_df, ct_ordered)
  } else {
