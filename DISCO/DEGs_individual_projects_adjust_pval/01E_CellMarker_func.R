...
CalcPercMarkers <- function(df_sex, markers, markers_ct_names, ct_order) {
  ...
  for (sex in names(df_sex)) {
    for (ct_name in names(df_sex[[sex]])) {
      df_markers[which(df_markers$ct==ct_name & df_markers$sex==sex),"markers_count"] <- length(intersect(df_sex[[sex]][[ct_name]]$Gene, markers[[data_ct[[ct_name]]]]))
      df_markers[which(df_markers$ct==ct_name & df_markers$sex==sex),"markers_perc"] <- length(intersect(df_sex[[sex]][[ct_name]]$Gene, markers[[data_ct[[ct_name]]]]))*100/length(markers[[data_ct[[ct_name]]]])
    }
  }
...
PlotMarkers <- function(main_dir, dis_type, df_markers) {
  out_path <- paste0(main_dir, dis_type, "/outputs/01E_CellMarker/")
  dir.create(out_path, showWarnings = FALSE, recursive = T)
  pdf(paste0(out_path, "CellMarker_count.pdf"))
  print(
  ...
  dev.off()
  pdf(paste0(out_path, "CellMarker_percentage.pdf"))
  print(
...
PlotCMresults <- function(main_dir, dis_type, cm_dir, cm_ct_list, markers_ct_names, ct_order, row_col) {
  ...
  if (sum(df_markers$markers_count) > 0 ) {
