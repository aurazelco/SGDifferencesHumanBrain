...
DfFrac <- function(main_dir, dis_type, cons_df, threshold, out_name, all_df) {
  out_path <- paste0(main_dir, dis_type, "/outputs/02C_Conservation/")
  ...
  write.csv(df_frac, paste0(out_path, out_name, "_fraction_in_", threshold,  "_species.csv"), row.names = F)
...
PlotFrac <- function(main_dir, dis_type, df_frac, threshold, out_name, ct_ordered) {
  ...
  pdf(paste0(out_path, out_name, "_fraction_in_", threshold,  "_species.pdf"))  
  print(
