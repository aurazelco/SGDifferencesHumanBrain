...
AREdf <- function(df_sex, sex, ARE_DF) {
  ...
  df_ARE$ct <- sapply(1:length(df_ARE$ct), function(i) str_replace(df_ARE$ct[i], paste0("_", sex, "_intersected_genes"), ""))
  df_ARE$bg <- sapply(1:length(names(df_sex)), function(i) length((unique(df_sex[[i]][, sex]))))
  df_ARE$full <- sapply(1:length(names(df_sex)), function(i) length(intersect(unique(df_sex[[i]][, sex]), unique(ARE_DF$fullsites))))
  df_ARE$half <- sapply(1:length(names(df_sex)), function(i) length(intersect(unique(df_sex[[i]][, sex]), unique(ARE_DF$halfsites))))
  df_ARE$hf <- sapply(1:length(names(df_sex)), function(i) length(intersect(unique(df_sex[[i]][, sex]), intersect(unique(ARE_DF$halfsites), unique(ARE_DF$fullsites)))))
...
EREdf <- function(df_sex, sex, ERE_gene) {
  ...
  df_ERE$ct <- sapply(1:length(df_ERE$ct), function(i) str_replace(df_ERE$ct[i], paste0("_", sex, "_intersected_genes"), ""))
  df_ERE$bg <- sapply(1:length(names(df_sex)), function(i) length((unique(df_sex[[i]][, sex]))))
  df_ERE$ERE_overlap <- sapply(1:length(names(df_sex)), function(i) length(intersect(unique(df_sex[[i]][, sex]), unique(ERE_gene))))
